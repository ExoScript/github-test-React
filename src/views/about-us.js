import React from 'react'

import { Helmet } from 'react-helmet'

import MenuBar from '../components/menu-bar'
import './about-us.css'

const AboutUs = (props) => {
  return (
    <div className="about-us-container">
      <Helmet>
        <title>About-us - Up2Data</title>
        <meta property="og:title" content="About-us - Up2Data" />
      </Helmet>
      <div className="about-us-container01">
        <div className="about-us-container02">
          <MenuBar></MenuBar>
          <div className="about-us-container03">
            <div className="about-us-container04">
              <div className="about-us-container05">
                <span className="titel-1">
                  An In-Depth Introduction to CRM Software and Its Functionality
                </span>
                <span className="text-2">
                  Moreover, CRM software aids in automating and optimizing
                  various business processes, from lead generation and sales
                  tracking to customer support and issue resolution.
                </span>
              </div>
              <div className="about-us-container06">
                <div className="about-us-container07">
                  <div className="about-us-container08">
                    <input
                      type="text"
                      placeholder="Email Address"
                      className="about-us-textinput input"
                    />
                  </div>
                  <div className="about-us-container09 gradient">
                    <span>Start free trail</span>
                  </div>
                </div>
                <span>30 day free trial - No credit card required</span>
              </div>
            </div>
            <div className="about-us-container10">
              <div className="about-us-container11">
                <div className="about-us-container12">
                  <span className="about-us-text04 sub-titel-1">ABOUT US</span>
                  <span className="titel-2">About Up2Data</span>
                  <span className="text-2">
                    <span>
                      Create a seamless integration between your CRM system and
                      maintain a constant flow of new highly qualified leads.
                    </span>
                    <br></br>
                    <span>
                      Moreover, CRM software aids in automating and optimizing
                      various business processes, from lead generation and sales
                      tracking to customer support and issue resolution. This
                      automation not only enhances efficiency but also reduces
                      the likelihood of errors and oversights, leading to
                      smoother operations and improved customer experiences.
                      <span
                        dangerouslySetInnerHTML={{
                          __html: ' ',
                        }}
                      />
                    </span>
                  </span>
                </div>
                <div className="about-us-container13">
                  <span className="about-us-text10">Get Started</span>
                  <svg viewBox="0 0 1024 1024" className="about-us-icon">
                    <path d="M481.835 243.499l225.835 225.835h-494.336c-23.552 0-42.667 19.115-42.667 42.667s19.115 42.667 42.667 42.667h494.336l-225.835 225.835c-16.683 16.683-16.683 43.691 0 60.331s43.691 16.683 60.331 0l298.667-298.667c3.925-3.925 7.083-8.619 9.259-13.824 4.309-10.453 4.309-22.229 0-32.683-2.091-5.035-5.163-9.728-9.259-13.824l-298.667-298.667c-16.683-16.683-43.691-16.683-60.331 0s-16.683 43.691 0 60.331z"></path>
                  </svg>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="about-us-container14">
          <div className="about-us-container15">
            <span className="about-us-text11">
              Our platform interacts with the CRMs that see the highest usage
              rates, ensuring optimal engagement.
            </span>
            <div className="about-us-container16">
              <span className="about-us-text12 titel-2">ClickUp</span>
              <span className="about-us-text13 titel-2">HubSpot</span>
              <span className="about-us-text14 titel-2">monday</span>
              <span className="about-us-text15 titel-2">Freshsales</span>
              <span className="about-us-text16 titel-2">salesforce</span>
              <span className="about-us-text17 titel-2">Less Annoying CRM</span>
              <span className="about-us-text18 titel-2">Zendesk</span>
              <span className="about-us-text19 titel-2">Active Campaign</span>
            </div>
          </div>
        </div>
        <div className="about-us-container17">
          <div className="about-us-container18">
            <div className="about-us-container19">
              <div className="about-us-container20">
                <div className="about-us-container21">
                  <span className="about-us-text20 sub-titel-1">
                    Our Mission
                  </span>
                  <span className="titel-2">
                    The Mission of Our CRM Software Solution
                  </span>
                </div>
              </div>
              <div className="about-us-container22">
                <div className="about-us-container23">
                  <span className="about-us-text22">Get Started</span>
                  <svg viewBox="0 0 1024 1024" className="about-us-icon02">
                    <path d="M481.835 243.499l225.835 225.835h-494.336c-23.552 0-42.667 19.115-42.667 42.667s19.115 42.667 42.667 42.667h494.336l-225.835 225.835c-16.683 16.683-16.683 43.691 0 60.331s43.691 16.683 60.331 0l298.667-298.667c3.925-3.925 7.083-8.619 9.259-13.824 4.309-10.453 4.309-22.229 0-32.683-2.091-5.035-5.163-9.728-9.259-13.824l-298.667-298.667c-16.683-16.683-43.691-16.683-60.331 0s-16.683 43.691 0 60.331z"></path>
                  </svg>
                </div>
              </div>
            </div>
            <div className="about-us-container24">
              <div className="about-us-container25">
                <img
                  alt="image"
                  src="/group%201413372149-900w.png"
                  className="about-us-image"
                />
              </div>
              <div className="about-us-container26">
                <span className="text-1">
                  <span>
                    One of the primary functions of CRM software is to
                    consolidate customer information from various sources, such
                    as email, social media, website interactions, and sales
                    transactions. This holistic view of customer data allows
                    companies to create
                    <span
                      dangerouslySetInnerHTML={{
                        __html: ' ',
                      }}
                    />
                  </span>
                  <span>
                    <span
                      dangerouslySetInnerHTML={{
                        __html: ' ',
                      }}
                    />
                  </span>
                </span>
                <div
                  data-thq="thq-dropdown"
                  className="about-us-thq-dropdown list-item"
                >
                  <ul
                    data-thq="thq-dropdown-list"
                    className="about-us-dropdown-list"
                  >
                    <li
                      data-thq="thq-dropdown"
                      className="about-us-dropdown list-item"
                    >
                      <div
                        data-thq="thq-dropdown-toggle"
                        className="about-us-dropdown-toggle"
                      >
                        <span className="about-us-text26">Sub-menu Item</span>
                      </div>
                    </li>
                    <li
                      data-thq="thq-dropdown"
                      className="about-us-dropdown1 list-item"
                    >
                      <div
                        data-thq="thq-dropdown-toggle"
                        className="about-us-dropdown-toggle1"
                      >
                        <span className="about-us-text27">Sub-menu Item</span>
                      </div>
                    </li>
                    <li
                      data-thq="thq-dropdown"
                      className="about-us-dropdown2 list-item"
                    >
                      <div
                        data-thq="thq-dropdown-toggle"
                        className="about-us-dropdown-toggle2"
                      >
                        <span className="about-us-text28">Sub-menu Item</span>
                      </div>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="about-us-container27">
          <div className="about-us-container28">
            <div className="about-us-container29">
              <span className="titel-2">
                Learn about our company success stories
              </span>
              <span className="about-us-text30 text-1">
                Start your sales journey today to get the most out of your
                efforts
              </span>
            </div>
            <div className="about-us-container30">
              <div className="about-us-container31">
                <div className="about-us-container32">
                  <div className="about-us-container33">
                    <div className="about-us-container34">
                      <img
                        alt="image"
                        src="/vector.svg"
                        className="about-us-image1"
                      />
                      <span className="about-us-text31">10K+</span>
                    </div>
                  </div>
                  <span className="about-us-text32">Sales Calls</span>
                </div>
                <div className="about-us-container35"></div>
                <div className="about-us-container36">
                  <div className="about-us-container37">
                    <div className="about-us-container38">
                      <img
                        alt="image"
                        src="/vector.svg"
                        className="about-us-image2"
                      />
                      <span className="about-us-text33">125K+</span>
                    </div>
                  </div>
                  <span className="about-us-text34">Data enriched</span>
                </div>
                <div className="about-us-container39"></div>
                <div className="about-us-container40">
                  <div className="about-us-container41">
                    <div className="about-us-container42">
                      <img
                        alt="image"
                        src="/vector.svg"
                        className="about-us-image3"
                      />
                      <span className="about-us-text35">15+</span>
                    </div>
                  </div>
                  <span className="about-us-text36">Years Experience</span>
                </div>
                <div className="about-us-container43"></div>
                <div className="about-us-container44">
                  <div className="about-us-container45">
                    <div className="about-us-container46">
                      <img
                        alt="image"
                        src="/vector.svg"
                        className="about-us-image4"
                      />
                      <span className="about-us-text37">8+</span>
                    </div>
                  </div>
                  <span className="about-us-text38">ROI on average</span>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="about-us-container47">
          <div className="about-us-container48">
            <div className="about-us-container49">
              <div className="about-us-container50">
                <span className="about-us-text39 sub-titel-1">Our Vision</span>
                <span className="titel-2">
                  Our Vision for Innovating CRM Software Solutions
                </span>
                <span className="text-1">
                  <span>
                    One of the primary functions of CRM software is to
                    consolidate customer information from various sources, such
                    as email, social media, website interactions, and sales
                    transactions.
                  </span>
                  <span>
                    <span
                      dangerouslySetInnerHTML={{
                        __html: ' ',
                      }}
                    />
                  </span>
                </span>
              </div>
              <div className="about-us-container51">
                <div className="about-us-container52">
                  <div className="about-us-container53">
                    <svg viewBox="0 0 1024 1024" className="about-us-icon04">
                      <path d="M954.857 323.429c0 14.286-5.714 28.571-16 38.857l-491.429 491.429c-10.286 10.286-24.571 16-38.857 16s-28.571-5.714-38.857-16l-284.571-284.571c-10.286-10.286-16-24.571-16-38.857s5.714-28.571 16-38.857l77.714-77.714c10.286-10.286 24.571-16 38.857-16s28.571 5.714 38.857 16l168 168.571 374.857-375.429c10.286-10.286 24.571-16 38.857-16s28.571 5.714 38.857 16l77.714 77.714c10.286 10.286 16 24.571 16 38.857z"></path>
                    </svg>
                  </div>
                  <span className="ueberschrift-2">
                    Automated AI based Cold Outreach
                  </span>
                </div>
                <div className="about-us-container54">
                  <div className="about-us-container55">
                    <svg viewBox="0 0 1024 1024" className="about-us-icon06">
                      <path d="M954.857 323.429c0 14.286-5.714 28.571-16 38.857l-491.429 491.429c-10.286 10.286-24.571 16-38.857 16s-28.571-5.714-38.857-16l-284.571-284.571c-10.286-10.286-16-24.571-16-38.857s5.714-28.571 16-38.857l77.714-77.714c10.286-10.286 24.571-16 38.857-16s28.571 5.714 38.857 16l168 168.571 374.857-375.429c10.286-10.286 24.571-16 38.857-16s28.571 5.714 38.857 16l77.714 77.714c10.286 10.286 16 24.571 16 38.857z"></path>
                    </svg>
                  </div>
                  <span className="ueberschrift-2">
                    CRM Analysis for more Sales Opportunities
                  </span>
                </div>
                <div className="about-us-container56">
                  <div className="about-us-container57">
                    <svg viewBox="0 0 1024 1024" className="about-us-icon08">
                      <path d="M954.857 323.429c0 14.286-5.714 28.571-16 38.857l-491.429 491.429c-10.286 10.286-24.571 16-38.857 16s-28.571-5.714-38.857-16l-284.571-284.571c-10.286-10.286-16-24.571-16-38.857s5.714-28.571 16-38.857l77.714-77.714c10.286-10.286 24.571-16 38.857-16s28.571 5.714 38.857 16l168 168.571 374.857-375.429c10.286-10.286 24.571-16 38.857-16s28.571 5.714 38.857 16l77.714 77.714c10.286 10.286 16 24.571 16 38.857z"></path>
                    </svg>
                  </div>
                  <span className="ueberschrift-2">
                    High Quality leads directly in your CRM
                  </span>
                </div>
                <div className="about-us-container58">
                  <div className="about-us-container59">
                    <svg viewBox="0 0 1024 1024" className="about-us-icon10">
                      <path d="M954.857 323.429c0 14.286-5.714 28.571-16 38.857l-491.429 491.429c-10.286 10.286-24.571 16-38.857 16s-28.571-5.714-38.857-16l-284.571-284.571c-10.286-10.286-16-24.571-16-38.857s5.714-28.571 16-38.857l77.714-77.714c10.286-10.286 24.571-16 38.857-16s28.571 5.714 38.857 16l168 168.571 374.857-375.429c10.286-10.286 24.571-16 38.857-16s28.571 5.714 38.857 16l77.714 77.714c10.286 10.286 16 24.571 16 38.857z"></path>
                    </svg>
                  </div>
                  <span className="ueberschrift-2">
                    Continuous Expert Support
                  </span>
                </div>
              </div>
              <div className="about-us-container60">
                <span className="about-us-text48">Get Started</span>
                <svg viewBox="0 0 1024 1024" className="about-us-icon12">
                  <path d="M481.835 243.499l225.835 225.835h-494.336c-23.552 0-42.667 19.115-42.667 42.667s19.115 42.667 42.667 42.667h494.336l-225.835 225.835c-16.683 16.683-16.683 43.691 0 60.331s43.691 16.683 60.331 0l298.667-298.667c3.925-3.925 7.083-8.619 9.259-13.824 4.309-10.453 4.309-22.229 0-32.683-2.091-5.035-5.163-9.728-9.259-13.824l-298.667-298.667c-16.683-16.683-43.691-16.683-60.331 0s-16.683 43.691 0 60.331z"></path>
                </svg>
              </div>
            </div>
            <div className="about-us-container61">
              <img
                alt="image"
                src="/group%201413372216-700h.png"
                className="about-us-image5"
              />
            </div>
          </div>
        </div>
        <div className="about-us-container62">
          <div className="about-us-container63">
            <div className="about-us-container64">
              <span className="titel-2">Subscribe Our Newsletter</span>
              <span className="about-us-text50 text-1">
                <span>
                  Enter your details to get business inspiration, trending
                  solutions,
                </span>
                <br></br>
                <span>and consulting tips delivered to your inbox</span>
              </span>
            </div>
            <div className="about-us-container65">
              <div className="about-us-container66">
                <span className="about-us-text54">Enter your email</span>
              </div>
              <div className="about-us-container67">
                <span className="about-us-text55">Subscribe</span>
                <svg viewBox="0 0 1024 1024" className="about-us-icon14">
                  <path d="M481.835 243.499l225.835 225.835h-494.336c-23.552 0-42.667 19.115-42.667 42.667s19.115 42.667 42.667 42.667h494.336l-225.835 225.835c-16.683 16.683-16.683 43.691 0 60.331s43.691 16.683 60.331 0l298.667-298.667c3.925-3.925 7.083-8.619 9.259-13.824 4.309-10.453 4.309-22.229 0-32.683-2.091-5.035-5.163-9.728-9.259-13.824l-298.667-298.667c-16.683-16.683-43.691-16.683-60.331 0s-16.683 43.691 0 60.331z"></path>
                </svg>
              </div>
            </div>
          </div>
        </div>
        <div className="about-us-container68">
          <div className="about-us-container69">
            <div className="about-us-container70">
              <div className="about-us-container71">
                <div className="about-us-container72"></div>
                <span className="about-us-text56">
                  <span>Drive Sales by thinking</span>
                  <br></br>
                  <span>outside the box</span>
                </span>
              </div>
              <div className="about-us-container73">
                <span>FOLLOW US</span>
                <div className="about-us-container74">
                  <div className="about-us-container75">
                    <svg
                      viewBox="0 0 602.2582857142856 1024"
                      className="about-us-icon16"
                    >
                      <path d="M548 6.857v150.857h-89.714c-70.286 0-83.429 33.714-83.429 82.286v108h167.429l-22.286 169.143h-145.143v433.714h-174.857v-433.714h-145.714v-169.143h145.714v-124.571c0-144.571 88.571-223.429 217.714-223.429 61.714 0 114.857 4.571 130.286 6.857z"></path>
                    </svg>
                  </div>
                  <div className="about-us-container76">
                    <svg
                      viewBox="0 0 950.8571428571428 1024"
                      className="about-us-icon18"
                    >
                      <path d="M925.714 233.143c-25.143 36.571-56.571 69.143-92.571 95.429 0.571 8 0.571 16 0.571 24 0 244-185.714 525.143-525.143 525.143-104.571 0-201.714-30.286-283.429-82.857 14.857 1.714 29.143 2.286 44.571 2.286 86.286 0 165.714-29.143 229.143-78.857-81.143-1.714-149.143-54.857-172.571-128 11.429 1.714 22.857 2.857 34.857 2.857 16.571 0 33.143-2.286 48.571-6.286-84.571-17.143-148-91.429-148-181.143v-2.286c24.571 13.714 53.143 22.286 83.429 23.429-49.714-33.143-82.286-89.714-82.286-153.714 0-34.286 9.143-65.714 25.143-93.143 90.857 112 227.429 185.143 380.571 193.143-2.857-13.714-4.571-28-4.571-42.286 0-101.714 82.286-184.571 184.571-184.571 53.143 0 101.143 22.286 134.857 58.286 41.714-8 81.714-23.429 117.143-44.571-13.714 42.857-42.857 78.857-81.143 101.714 37.143-4 73.143-14.286 106.286-28.571z"></path>
                    </svg>
                  </div>
                  <div className="about-us-container77">
                    <svg viewBox="0 0 1024 1024" className="about-us-icon20">
                      <path d="M406.286 644.571l276.571-142.857-276.571-144.571v287.429zM512 152c215.429 0 358.286 10.286 358.286 10.286 20 2.286 64 2.286 102.857 43.429 0 0 31.429 30.857 40.571 101.714 10.857 82.857 10.286 165.714 10.286 165.714v77.714s0.571 82.857-10.286 165.714c-9.143 70.286-40.571 101.714-40.571 101.714-38.857 40.571-82.857 40.571-102.857 42.857 0 0-142.857 10.857-358.286 10.857v0c-266.286-2.286-348-10.286-348-10.286-22.857-4-74.286-2.857-113.143-43.429 0 0-31.429-31.429-40.571-101.714-10.857-82.857-10.286-165.714-10.286-165.714v-77.714s-0.571-82.857 10.286-165.714c9.143-70.857 40.571-101.714 40.571-101.714 38.857-41.143 82.857-41.143 102.857-43.429 0 0 142.857-10.286 358.286-10.286v0z"></path>
                    </svg>
                  </div>
                  <div className="about-us-container78">
                    <svg viewBox="0 0 1024 1024" className="about-us-icon22">
                      <path d="M512 92.2c136.8 0 153 0.6 206.8 3 50 2.2 77 10.6 95 17.6 23.8 9.2 41 20.4 58.8 38.2 18 18 29 35 38.4 58.8 7 18 15.4 45.2 17.6 95 2.4 54 3 70.2 3 206.8s-0.6 153-3 206.8c-2.2 50-10.6 77-17.6 95-9.2 23.8-20.4 41-38.2 58.8-18 18-35 29-58.8 38.4-18 7-45.2 15.4-95 17.6-54 2.4-70.2 3-206.8 3s-153-0.6-206.8-3c-50-2.2-77-10.6-95-17.6-23.8-9.2-41-20.4-58.8-38.2-18-18-29-35-38.4-58.8-7-18-15.4-45.2-17.6-95-2.4-54-3-70.2-3-206.8s0.6-153 3-206.8c2.2-50 10.6-77 17.6-95 9.2-23.8 20.4-41 38.2-58.8 18-18 35-29 58.8-38.4 18-7 45.2-15.4 95-17.6 53.8-2.4 70-3 206.8-3zM512 0c-139 0-156.4 0.6-211 3-54.4 2.4-91.8 11.2-124.2 23.8-33.8 13.2-62.4 30.6-90.8 59.2-28.6 28.4-46 57-59.2 90.6-12.6 32.6-21.4 69.8-23.8 124.2-2.4 54.8-3 72.2-3 211.2s0.6 156.4 3 211c2.4 54.4 11.2 91.8 23.8 124.2 13.2 33.8 30.6 62.4 59.2 90.8 28.4 28.4 57 46 90.6 59 32.6 12.6 69.8 21.4 124.2 23.8 54.6 2.4 72 3 211 3s156.4-0.6 211-3c54.4-2.4 91.8-11.2 124.2-23.8 33.6-13 62.2-30.6 90.6-59s46-57 59-90.6c12.6-32.6 21.4-69.8 23.8-124.2 2.4-54.6 3-72 3-211s-0.6-156.4-3-211c-2.4-54.4-11.2-91.8-23.8-124.2-12.6-34-30-62.6-58.6-91-28.4-28.4-57-46-90.6-59-32.6-12.6-69.8-21.4-124.2-23.8-54.8-2.6-72.2-3.2-211.2-3.2v0z"></path>
                      <path d="M512 249c-145.2 0-263 117.8-263 263s117.8 263 263 263 263-117.8 263-263c0-145.2-117.8-263-263-263zM512 682.6c-94.2 0-170.6-76.4-170.6-170.6s76.4-170.6 170.6-170.6c94.2 0 170.6 76.4 170.6 170.6s-76.4 170.6-170.6 170.6z"></path>
                      <path d="M846.8 238.6c0 33.91-27.49 61.4-61.4 61.4s-61.4-27.49-61.4-61.4c0-33.91 27.49-61.4 61.4-61.4s61.4 27.49 61.4 61.4z"></path>
                    </svg>
                  </div>
                </div>
              </div>
            </div>
            <div className="about-us-container79">
              <div className="about-us-container80">
                <span className="about-us-text61">EXPLORE</span>
                <span className="about-us-text62">Home</span>
                <span className="about-us-text63">About Us</span>
                <span className="about-us-text64">Service</span>
                <span className="about-us-text65">Product</span>
                <span className="about-us-text66">Pricing</span>
                <span className="about-us-text67">Resources</span>
              </div>
              <div className="about-us-container81">
                <span className="about-us-text68">SUPPORT</span>
                <span className="about-us-text69">Help</span>
                <span className="about-us-text70">Contact Us</span>
                <span className="about-us-text71">Careers</span>
                <span className="about-us-text72">Terms Of Service</span>
                <span className="about-us-text73">Refund Policy</span>
                <span className="about-us-text74">Partners</span>
              </div>
              <div className="about-us-container82">
                <span className="about-us-text75">Contact info</span>
                <span className="about-us-text76">
                  <span>(702) 555-0122</span>
                  <br></br>
                  <span>(702) 555-0122</span>
                </span>
                <span className="about-us-text80">Up2Data@example.com</span>
                <span className="about-us-text81">
                  <span>3891 Ranchview Dr. Richardson,</span>
                  <br></br>
                  <span>California 62639</span>
                </span>
              </div>
            </div>
          </div>
        </div>
        <div className="about-us-container83">
          <div className="about-us-container84">
            <span>© 2023 Up2Data. All Rights Reserved</span>
          </div>
        </div>
      </div>
    </div>
  )
}

export default AboutUs
